...
This will extract all language into i18n JSON files in `client/translations`.
...
You can remove `react-intl` modules by first removing the `IntlProvider` object from the `client/app.js` file and by either removing or not selecting the i18n text option during component scaffolding.
